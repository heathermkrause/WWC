...
process_acs_age <- function(geographyfetch, yearspan = 1) {
        ...
        totalage <- bind_rows(totalage %>% group_by(sex) %>% 
        ...
        age <- bind_rows(age %>% group_by(sex, raceethnicity) %>% 
                                      filter(age == "Under 5 years" |
                                                     age == "5 to 9 years" |
                                                     age == "10 to 14 years" |
                                                     age == "15 to 17 years") %>% 
                                      summarize(population = sum(population, na.rm = TRUE)) %>% 
                                      mutate(age = "Under 18 years"),
                              age %>% group_by(sex, raceethnicity) %>% 
                                      filter(age == "18 and 19 years" | 
                                                     age == "20 to 24 years") %>% 
                                      summarize(population = sum(population, na.rm = TRUE)) %>% 
                                      mutate(age = "18 to 24 years"),
                              ...
                              age %>% group_by(sex, raceethnicity) %>% 
                                      ...
                                      mutate(age = "65 years and over")) %>%
                ungroup %>%
                select(sex, raceethnicity, age, population)
